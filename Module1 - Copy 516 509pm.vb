Module Module1
    ...
    Property rotationbluropacity As Double = 0.05
    Property rotationblurxpos As Double = 0.5
    Property rotationblurypos As Double = 0.5
    Property distortionblurpasses As Integer = 50
    Property distortionblurvariance As Integer = 2
    Property distortionbluropacity As Double = 0.05
    ...
    Sub Main()
        ...
        mustbeusedalone.Add(ColorChannels.distortionblur)
        ...
        If args(0).IndexOf("-") = 0 Then
            ...
            For Each itm As String In allowstring
                ...
                Select Case itm
                    ...
                    Case "j"
                        If allowstring.IndexOf("j") = 1 And allowstring.Length = 2 Then
                            allowonly.Add(GetType(ColorChannels).GetEnumName(ColorChannels.distortionblur))
                        Else
                            Console.WriteLine("DistortionBlur must be used alone, ignoring command")
                    ...
                    Select Case firstlet
                        ...
                        Case "o"
                            Try
                                ...
                                If imgtransparency < 0 Then
                                    imgtransparency = 0
                                End If

                                If imgtransparency > 1 Then
                                    imgtransparency = 1
                                End If
                            Catch ex As Exception
                        ...
                        Case "s"
                            Try
                                ...
                                If scanlinepower > 1 Then
                                    scanlinepower = 1
                                End If
                                If scanlinepower < 0 Then
                                    scanlinepower = 0
                                End If
                            Catch ex As Exception
                        ...
                        Case "k"
                            Try
                                rotationbluropacity = Double.Parse(bsub / 100)
                                If rotationbluropacity > 1 Then
                                    rotationbluropacity = 1
                                End If
                                If rotationbluropacity < 0 Then
                                    rotationbluropacity = 0
                                End If
                            Catch ex As Exception
                        ...
                        Case "K"
                            Try
                                rotationblurpasses = Integer.Parse(bsub)
                                If rotationblurpasses < 1 Then
                                    rotationblurpasses = 1
                                End If
                            Catch ex As Exception
                        ...
                        Case "v"
                            Try
                                rotationblurvariance = Integer.Parse(bsub)
                                If rotationblurvariance < 1 Then
                                    rotationblurvariance = 1
                                End If
                            Catch ex As Exception
                        ...
                        Case "V"
                            Try
                                rotationblurxpos = Double.Parse(bsub)
                            Catch ex As Exception

                            End Try
                        Case "y"
                            Try
                                rotationblurypos = Double.Parse(bsub)
                            Catch ex As Exception

                            End Try
                        Case "Y"
                            Try
                                distortionblurpasses = Integer.Parse(bsub)
                                If distortionblurpasses < 1 Then
                                    distortionblurpasses = 1
                                End If
                            Catch ex As Exception
                        ...
                        Case "j"
                            Try
                                distortionbluropacity = Double.Parse(bsub) / 100
                                If distortionbluropacity > 1 Then
                                    distortionbluropacity = 1

                                End If
                                If distortionbluropacity < 0 Then
                                    distortionbluropacity = 0
                                End If
                            Catch ex As Exception
                        ...
                        Case "J"
                            Try
                                distortionblurvariance = Integer.Parse(bsub)
                                If distortionblurvariance < 1 Then
                                    distortionblurvariance = 1
                                End If
                            Catch ex As Exception
        ...
        For Each eval As Integer In GetType(ColorChannels).GetEnumValues
            ...
            If allowances.Count > 0 Then
                ...
                If allowances.Count > 1 Then
                    Select Case eval
                        Case ColorChannels.scanlines, ColorChannels.linedraw, ColorChannels.rotateimage, ColorChannels.fade, ColorChannels.crosshatch, ColorChannels.distortionblur, ColorChannels.rotationblur, ColorChannels.pallette
                            Continue For
                    End Select
                End If
            Else
    ...
    Sub splitIntochannels(cob As Object)
            ...
            For x As Integer = 0 To pictosplit.Width - 1
                ...
                Select Case chanselect
                    ...
                    Case ColorChannels.rotationblur
                        ...
                        Dim centerImg As New Drawing.Point(pictosplit.Width * rotationblurxpos, pictosplit.Height * rotationblurxpos)
                        grip.MakeTransparent()

                        For yn As Integer = 0 To grip.Height - 1
                            For xn As Integer = 0 To grip.Width - 1
                                grip.SetPixel(xn, yn, Drawing.Color.FromArgb(Convert.ToInt32(255 * rotationbluropacity), pictosplit.GetPixel(xn, yn)))
                            Next
                        Next
                    ...
                    Case ColorChannels.distortionblur
                        If usecoloralpha Then
                            imglist.Add(savename, Nothing)
                            writecolor.Dispose()
                            dgui.Dispose()
                        ...
                        For yn As Integer = 0 To grip.Height - 1
                            For xn As Integer = 0 To grip.Width - 1
                                grip.SetPixel(xn, yn, Drawing.Color.FromArgb(Convert.ToInt32(255 * distortionbluropacity), pictosplit.GetPixel(xn, yn)))
                            Next
                        Next

                        Dim rmath As New Random

                        For xo As Integer = 1 To distortionblurpasses
                            dgui.DrawImageUnscaled(grip, New Drawing.Point(rmath.Next(-1 * distortionblurvariance, distortionblurvariance), rmath.Next(-1 * distortionblurvariance, distortionblurvariance)))
                        Next
        ...
        Try
            pictosplit.Dispose()
            saveImageAsFile(writecolor, tfile)
            writecolor.Dispose()
    ...
    Sub saveImageAsFile(img As Drawing.Image, path As String)
        Dim myEncoderParameters As New Drawing.Imaging.EncoderParameters(1)
        Dim myEncoder As System.Drawing.Imaging.Encoder = System.Drawing.Imaging.Encoder.Quality
        Dim myEncoderParameter As New System.Drawing.Imaging.EncoderParameter(myEncoder, 100&)
        myEncoderParameters.Param(0) = myEncoderParameter
        Dim pngencoder As Drawing.Imaging.ImageCodecInfo = GetEncoder(Drawing.Imaging.ImageFormat.Png)
        img.Save(path, pngencoder, myEncoderParameters)
    End Sub
End Module
